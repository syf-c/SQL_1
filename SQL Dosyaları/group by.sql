...
select isim,sum(urun_miktari)from manav group by isim    --alınanmeyvelerintolamı kişi ismine göre
select isim,sum(urun_miktari) as toplam_urun from manav group by isim --alınan meyvelerin toplamı kişi ismine göre
select urun_adi, count(isim)from manav  where urun_adi is not null group by urun_adi

-- satılan meyve türüne (urun_adi) göre satılan (urun_miktari )MIN ve -- MAX urun miktarlarini, MAX urun miktarina 
--göre sıralayarak listeyen sorguyu yazınız
...
select ulke                                        from personel group by ulke --- ulke ülkeleri gruplandırır
select ulke, round(avg(maas)                       from personel group by ulke -- ülkelerin maas ortalamalarına göre
select cinsiyet ,avg(maas)                         from personel group by cinsiyet -- cinsiyetlerinmaas ortalaması
select cinsiyet ,round(avg(maas)                   from personel group by cinsiyet  -- cinsiyetlerinmaas ortalaması
select ulke,sehir                                  from personel group by ulke,sehir --sehirleri ülkelere guruplandırdı rasgele
select ulke,sehir,count(sehir)                     from personel group by ulke,sehir   --PERSONELİN ŞEHİRLERE GÖRE DAĞILIMI
select ulke,sehir,count(sehir) as C_SAYISI         from personel group by ulke,sehir
select ulke,cinsiyet,count(*)  as C_sayısı,avg(yas)as Y_Ort from personel group by ulke,cinsiyet
