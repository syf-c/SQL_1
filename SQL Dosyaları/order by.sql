...
select * from kisiler order by adres  --adrese göre sıralar
select*from kisiler order by adres asc;  -- adrese göre aynı sıralama
select * from kisiler order by adres desc --tersden sıralama

select * from kisiler order by maas  --maasa göre büyükten küçüğe sıralama
select *from kisiler where isim='Mine' order by maas  desc --minelerin maasa göre sıralanması
select *from kisiler where soyisim='Bulut' order by maas  -- soyismi bulut olanlarınmaas sıralanması
